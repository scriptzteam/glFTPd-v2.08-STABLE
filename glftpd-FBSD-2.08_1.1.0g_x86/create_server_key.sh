...
if [ -z "$1" ] ; then
	echo "Usage: ./create_server_key.sh [rsa/dsa/ecdsa(default)] info"
        echo "info - can be any word, and it should inform the client"
        echo "       about the server he is logging in (for example servername)"
        echo "rsa/dsa/ecdsa - if you dont specify this then ECDSA key will be created"
...
if [ $type = "dsa" ]; then 
        $opensslbin rand -out gunk $keysize
	$opensslbin dsaparam -rand gunk -out $base.dsaparam $keysize
	$opensslbin dhparam -rand gunk -out $base.dh $keysize
        rm -f gunk
	$opensslbin gen$type -out $base.key $base.dsaparam
elif [ $type = "rsa" ]; then
